...
// op_timeout composes the primitives: a cpu_timeout bounds the launch, and once
// the start event completes (via stream_completed) its callback enqueues a
// stream_timeout that bounds the completion. The user callback fires at most
// once. The returned handle is the launch timer; the caller cancels it once the
// launch (e.g. the guarded enqueue) is done, while the completion monitoring
// set up here continues independently.
Handle Watchdog::op_timeout(
    c10::Stream stream,
    std::chrono::milliseconds timeout,
    Callback callback) {
  auto weak = weak_from_this();
  auto fired = std::make_shared<std::atomic<bool>>(false);
  Callback once = [fired, callback = std::move(callback)]() {
    if (!fired->exchange(true)) {
      callback();
    }
  };

  stream_completed(stream, [weak, stream, timeout, once]() {
    if (auto self = weak.lock()) {
      self->stream_timeout(stream, timeout, once);
    }
  });
  return cpu_timeout(timeout, once);
}
...
class WatchdogImpl : public Watchdog {
 ...
 private:
  ...
  class EventCache {
   public:
    std::shared_ptr<c10::Event> acquire(const c10::Stream& stream) {
      {
        ...
        auto& free = free_[stream.device_index()];
        if (!free.empty()) {
    ...
    void release(std::shared_ptr<c10::Event> event) {
      ...
      std::lock_guard<std::mutex> lock(mutex_);
      free_[event->device_index()].push_back(std::move(event));
    }
   ...
   private:
    ...
    std::unordered_map<
        c10::DeviceIndex,
        std::vector<std::shared_ptr<c10::Event>>>
        free_;
  };
  ...
  void updateActiveCount() {
    activeCount_.store(streamTimeouts_.size() + streamCompleted_.size());
  }
...
void WatchdogImpl::maybeStopPollTimer() {
  if (streamTimeouts_.empty() && streamCompleted_.empty() && pollActive_) {
...
Handle Watchdog::makeHandle(uint64_t id) {
  return Handle(weak_from_this(), id);
}

std::unique_ptr<Watchdog> makeWatchdog(std::chrono::milliseconds pollInterval) {
  return std::make_unique<WatchdogImpl>(pollInterval);
}
